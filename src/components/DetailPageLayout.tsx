...
export const DetailPageLayout = component$<DetailPageLayoutProps>(({
  ...
  const prevSlide = $(() => {
    currentSlide.value = currentSlide.value === 0 ? galleryImages.length - 1 : currentSlide.value - 1;
  });
          ...
          <div class="space-y-6">
            <div class="gap-4 grid grid-cols-2">
              <div class="bg-base-200 p-4 border border-base-300 rounded-lg">
                <div class="mb-1 text-xs text-base-content/50 uppercase tracking-wide">Room Type</div>
                <div class="font-semibold text-primary">{metadata.roomType || 'Standard'}</div>
              </div>
              <div class="bg-base-200 p-4 border border-base-300 rounded-lg">
                <div class="mb-1 text-xs text-base-content/50 uppercase tracking-wide">Max Occupancy</div>
            ...
            {metadata.sizeSqm && (
              <div class="bg-base-200 p-4 border border-base-300 rounded-lg">
                <div class="mb-1 text-xs text-base-content/50 uppercase tracking-wide">Room Size</div>
            ...
            {metadata.bedConfiguration && (
              <div class="bg-base-200 p-4 border border-base-300 rounded-lg">
                <div class="mb-1 text-xs text-base-content/50 uppercase tracking-wide">Bed Configuration</div>
              ...
              <div>
                ...
                <div class="flex flex-wrap gap-2">
                  {metadata.amenities.map((amenity, index) => (
                    <span key={index} class="bg-primary/10 px-3 py-1 rounded-full text-primary text-sm">
                      {amenity}
                    </span>
                  ))}
                </div>
      ...
      case 'activity':
        return (
          <div class="space-y-6">
            <div class="gap-4 grid grid-cols-2">
              <div class="bg-base-200 p-4 border border-base-300 rounded-lg">
                <div class="mb-1 text-xs text-base-content/50 uppercase tracking-wide">Duration</div>
                <div class="font-semibold text-primary">
                  {metadata.durationMinutes ? `${metadata.durationMinutes} min` : 'Varies'}
                </div>
              </div>
              <div class="bg-base-200 p-4 border border-base-300 rounded-lg">
                <div class="mb-1 text-xs text-base-content/50 uppercase tracking-wide">Difficulty</div>
                <div class="font-semibold text-primary">{metadata.difficultyLevel || 'All levels'}</div>
              </div>
            </div>

            {metadata.maxParticipants && (
              <div class="bg-base-200 p-4 border border-base-300 rounded-lg">
                <div class="mb-1 text-xs text-base-content/50 uppercase tracking-wide">Max Participants</div>
            ...
            {metadata.location && (
              <div class="bg-base-200 p-4 border border-base-300 rounded-lg">
                <div class="mb-1 text-xs text-base-content/50 uppercase tracking-wide">Location</div>
            ...
            {metadata.price && (
              <div class="bg-accent/10 p-6 border border-accent/20 rounded-lg">
                ...
                <div class="font-bold text-accent text-3xl">{metadata.price} {defaultCurrency}</div>
      ...
      case 'facility':
        return (
          <div class="space-y-6">
            <div class="gap-4 grid grid-cols-2">
              <div class="bg-base-200 p-4 border border-base-300 rounded-lg">
                <div class="mb-1 text-xs text-base-content/50 uppercase tracking-wide">Capacity</div>
                <div class="font-semibold text-primary">
                  {metadata.capacity ? `${metadata.capacity} people` : 'Unlimited'}
                </div>
              </div>
              <div class="bg-base-200 p-4 border border-base-300 rounded-lg">
                <div class="mb-1 text-xs text-base-content/50 uppercase tracking-wide">Category</div>
                <div class="font-semibold text-primary">{metadata.facilityCategory || 'General'}</div>
              </div>
            </div>
            ...
            {metadata.operatingHours && (
              <div class="bg-base-200 p-4 border border-base-300 rounded-lg">
                <div class="mb-1 text-xs text-base-content/50 uppercase tracking-wide">Operating Hours</div>
            ...
            {metadata.pricePerHour && (
              <div class="bg-accent/10 p-6 border border-accent/20 rounded-lg">
                <div class="mb-1 text-accent text-xs uppercase tracking-wide">Price</div>
                <div class="font-bold text-accent text-3xl">{metadata.pricePerHour} {defaultCurrency}</div>
            ...
            {metadata.facilityIsAccessible && (
              <div class="bg-info/10 p-4 border border-info/20 rounded-lg">
                ...
                <div class="mt-1 text-info/80 text-sm">This facility is wheelchair accessible</div>
              </div>
            )}
          </div>
        );
          ...
          <div class="flex items-center space-x-2 text-sm">
            {breadcrumbItems.map((item, index) => (
              <div key={index} class="flex items-center">
                ...
                {item.href ? (
                  <a href={item.href} class="text-primary hover:text-primary-focus transition-colors">
                    {item.label}
                  </a>
                ) : (
        ...
        <div class="mx-auto max-w-7xl">
          <div class="gap-12 grid grid-cols-1 lg:grid-cols-3">
            ...
            <div class="space-y-8 lg:col-span-2">
              <div>
                <h2 class="mb-6 font-head font-bold text-3xl">About This {type.charAt(0).toUpperCase() + type.slice(1)}</h2>
                    ...
                    {galleryImages.map((media, index) => (
                      <button
                        ...
                        class={`relative flex-shrink-0 bg-base-200 border-2 w-20 h-16 overflow-hidden transition-colors ${
                          index === currentSlide.value ? 'border-primary' : 'border-transparent hover:border-primary'
                        }`}
                      >
                        {media.type === 'video' ? (
                          <video
            ...
            <div class="space-y-8">
              ...
              <div class="space-y-4">
                ...
                >
                  {type === 'room' ? 'Check Availability' : type === 'activity' ? 'Reserve Activity' : 'Book Facility'}
                </button>
                <button class="btn-outline w-full btn">
                  Contact Reception
                </button>
              </div>
          ...
          <div class="max-w-4xl modal-box">
            <div class="flex justify-between items-center mb-4">
              <h3 class="font-bold text-lg">
                {type === 'room' ? `Book ${title}` : type === 'activity' ? `Reserve ${title}` : `Book ${title}`}
              </h3>
              <button
              ...
              <div class="flex justify-between items-center">
                <div>
                  ...
                  <p class="opacity-70 text-sm">
                    {type === 'room' && metadata.roomType && `${metadata.roomType} • Max ${metadata.maxOccupancy} guests`}
                    {type === 'activity' && metadata.category && `${metadata.category} • ${metadata.durationMinutes} minutes`}
                    {type === 'facility' && metadata.facilityCategory && `${metadata.facilityCategory} • Capacity: ${metadata.capacity}`}
                  </p>
                </div>
                <div class="text-right">
                  {(metadata.basePrice || metadata.price || metadata.pricePerHour) && (
                    <div class="font-bold text-primary text-2xl">
                      {defaultCurrency} {metadata.basePrice || metadata.price || metadata.pricePerHour}
                    </div>
                  )}
                  <p class="opacity-70 text-sm">
                    {type === 'room' ? 'per night' : type === 'facility' ? 'per hour' : 'per person'}
                  </p>
                </div>
              </div>
            </div>
