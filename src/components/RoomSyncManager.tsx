...
export const RoomSyncManager = component$<RoomSyncManagerProps>(({
  ...
  const loadSyncStatus = $(async () => {
    ...
    try {
      const response = await fetch(`/api/booking/sync-room-ids?hotelDomain=${encodeURIComponent(hotelDomain)}`);
  ...
  const performAutoSync = $(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (successCount > 0) {
          alert(`Successfully synced ${successCount} room${successCount > 1 ? 's' : ''}!${errorCount > 0 ? ` ${errorCount} room${errorCount > 1 ? 's' : ''} had errors.` : ''}`);
        } else if (errorCount > 0) {
          alert(`Failed to sync ${errorCount} room${errorCount > 1 ? 's' : ''}. Please check the logs.`);
        } else {
            ...
            <div class="space-y-6">
              ...
              <div class="stats stats-vertical lg:stats-horizontal shadow">
                <div class="stat">
                  ...
                  <div class="stat-value text-primary">{syncData.value.syncStatus.totalRooms}</div>
                </div>
                ...
                <div>
                  ...
                  <div class="space-y-2">
                    {syncData.value.syncRecommendations.map((recommendation, index) => (
                      <div key={index} class="flex justify-between items-center p-3 bg-warning/10 rounded-lg">
                        <div>
              ...
              {syncResults.value.length > 0 && (
                <div>
                  <h4 class="font-semibold text-success mb-3">✅ Recent Sync Results</h4>
                  <div class="space-y-2">
                    {syncResults.value.map((result, index) => (
                      <div key={index} class={`flex justify-between items-center p-3 rounded-lg ${
                        result.status === 'updated' ? 'bg-success/10' : 'bg-error/10'
                      }`}>
                        <div>
                          <span class="font-medium">{result.roomName}</span>
                          {result.error && (
                            <p class="text-sm text-error mt-1">{result.error}</p>
                          )}
                        </div>
                        <div class="text-right">
                          <div class={`badge ${result.status === 'updated' ? 'badge-success' : 'badge-error'}`}>
                            {result.status}
                          </div>
                          {result.pmsRoomId && (
                            <div class="text-xs opacity-70 mt-1">
              ...
              {/* No Sync Needed */}
              {syncData.value.syncRecommendations.length === 0 && syncData.value.syncStatus.roomsWithoutPmsId === 0 && (
