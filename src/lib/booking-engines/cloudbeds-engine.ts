...
export class CloudbedsEngine implements IBookingEngine {
  ...
  private async makeRequest<T = CloudbedsApiResponse>(
    ...
    console.log("🌐 [CLOUDBEDS] Headers:", { ...headers, Authorization: `Bearer ${this.accessToken.substring(0, 10)}...` });
    if (options.body) {
    ...
    try {
      ...
      console.log("🌐 [CLOUDBEDS] Response status:", response.status, response.statusText);
      ...
      console.log("🌐 [CLOUDBEDS] Response data:", JSON.stringify(responseData, null, 2));
  ...
  async checkAvailability(query: RateQuery): Promise<RoomAvailability[]> {
    try {
      ...
      console.log("🏨 [CLOUDBEDS] Query parameters:", JSON.stringify(query, null, 2));
      console.log("🏨 [CLOUDBEDS] Property ID:", this.config?.credentials.propertyId);
      ...
      console.log("🏨 [CLOUDBEDS] Raw API response:", JSON.stringify(response, null, 2));
      ...
      console.log("🏨 [CLOUDBEDS] Transformed availability:", availability.length, "rooms");
  ...
  async getRates(query: RateQuery): Promise<RoomRate[]> {
    try {
      ...
      console.log("💰 [CLOUDBEDS] Raw rates response:", JSON.stringify(response, null, 2));
  ...
  async createBooking(request: BookingRequest): Promise<BookingResponse> {
    try {
      ...
      console.log("📝 [CLOUDBEDS] Booking request:", JSON.stringify(request, null, 2));

      const bookingData = this.transformBookingRequest(request);
      console.log("📝 [CLOUDBEDS] Transformed booking data:", JSON.stringify(bookingData, null, 2));
      ...
      console.log("📝 [CLOUDBEDS] Raw booking response:", JSON.stringify(response, null, 2));

      const transformedResponse = this.transformBookingResponse(response);
      console.log("📝 [CLOUDBEDS] Transformed booking response:", JSON.stringify(transformedResponse, null, 2));
  ...
  async getBooking(bookingId: string): Promise<Record<string, unknown>> {
    try {
      ...
      console.log("🔍 [CLOUDBEDS] Booking details response:", JSON.stringify(response, null, 2));
  ...
  async debugCloudbedsRooms(): Promise<unknown> {
    try {
      const response = await this.makeRequest("/getRooms");
      return response;
    } catch (error) {
      throw error;
    }
  ...
  async debugCloudbedsRoomTypes(): Promise<unknown> {
    try {
      const response = await this.makeRequest("/getRoomTypes");
      return response;
    } catch (error) {
      throw error;
    }
   ...
   */
  async processPayment(paymentRequest: CloudbedsPaymentRequest): Promise<CloudbedsPaymentResponse> {
    try {
      console.log("💳 [CLOUDBEDS] Starting payment processing...");
      console.log("💳 [CLOUDBEDS] Payment request:", JSON.stringify(paymentRequest, null, 2));

      // CORRECCIÓN: Usar form-urlencoded en lugar de JSON
      const formData = new URLSearchParams();
      formData.append('reservationID', paymentRequest.reservation_id.toString());
      ...
      console.log("💳 [CLOUDBEDS] Form data:", Object.fromEntries(formData.entries()));
      ...
      console.log("💳 [CLOUDBEDS] Payment headers:", { ...headers, Authorization: "Bearer ***" });
      ...
      try {
        ...
        console.error("❌ [CLOUDBEDS] Failed to parse payment response:", parseError);
        throw new BookingError(`Invalid JSON response: ${responseText}`, parseError);
      }

      console.log("💳 [CLOUDBEDS] Parsed payment response:", JSON.stringify(responseData, null, 2));
      ...
      const paymentResponse: CloudbedsPaymentResponse = {
        ...
        id: responseData.data?.paymentID || responseData.data?.id || Date.now().toString(),
        ...
        transaction_id: responseData.data?.transactionID || responseData.data?.transaction_id,
   ...
   */
  async getRatePlans(startDate?: string, endDate?: string): Promise<CloudbedsRatePlansResponse> {
    try {
      ...
      console.warn("⚠️ [FALLBACK] Cloudbeds rate plans API unavailable, using default rate plans");
      ...
      return {
        ...
        data: [
          ...
          },
          {
            ...
            description: "Room rate with complimentary breakfast for all guests",
            rate_type: "package",
            currency: "USD",
            is_active: true,
   ...
   */
  async getHotelDetails(): Promise<CloudbedsListResponse<CloudbedsPropertyResponse>> {
    try {
      const response = await this.makeRequest<CloudbedsListResponse<CloudbedsPropertyResponse>>(
        "/getHotels"
      );
      ...
      console.warn("⚠️ [FALLBACK] Cloudbeds hotel details API unavailable, using default details");
  ...
  private transformAvailabilityResponse(
    ...
    console.log("🔄 [CLOUDBEDS] Response data length:", response.data?.length || 0);
    ...
    for (const property of response.data) {
      ...
      console.log("🔄 [CLOUDBEDS] Property has", property.propertyRooms.length, "rooms");

      for (const room of property.propertyRooms) {
        const roomsAvailable = room.roomsAvailable || 0;
        console.log(`🔄 [CLOUDBEDS] Room ${room.roomTypeName} (${room.roomTypeID}): ${roomsAvailable} available`);
        ...
        if (roomsAvailable > 0) {
          ...
          console.log(`✅ [CLOUDBEDS] Added room: ${room.roomTypeName} ($${room.roomRate})`);
        } else {
          console.log(`❌ [CLOUDBEDS] Skipped room: ${room.roomTypeName} (${roomsAvailable} available)`);
        }
      }
    }
