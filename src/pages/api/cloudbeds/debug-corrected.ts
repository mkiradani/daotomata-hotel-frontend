...
export const GET: APIRoute = async () => {
  try {
    console.log("🔍 [DEBUG] Starting Cloudbeds debug with Directus credentials...");
    ...
    console.log("🔍 [DEBUG] Engine initialized with Directus credentials and API v1.3");

    // Test hotels endpoint first
    console.log("🔍 [DEBUG] Testing getHotels endpoint...");
    const hotelsResponse = await (engine as any).debugCloudbedsHotels();

    // Test rooms endpoint
    console.log("🔍 [DEBUG] Testing getRooms endpoint...");
    const roomsResponse = await (engine as any).debugCloudbedsRooms();
