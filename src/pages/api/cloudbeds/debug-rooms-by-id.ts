  ...
  try {
    ...
    try {
      ...
      const [roomsData, roomTypesData, hotelsData] = await Promise.allSettled([
        ...
        (engine as any).debugCloudbedsHotels(),
      ]);
      ...
      const response = {
        ...
        cloudbeds_data: {
          hotels: {
            ...
            error: hotelsData.status === 'rejected' ? hotelsData.reason?.message : null,
          },
          rooms: {
      ...
      console.error("❌ [DEBUG] Failed to initialize or call Cloudbeds:", initError);
      return new Response(
        JSON.stringify({
