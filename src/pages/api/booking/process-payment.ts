...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!paymentData.reservation_id || !paymentData.amount || !paymentData.currency || !paymentData.payment_method_id) {
      return new Response(JSON.stringify({
        success: false,
        error: "Missing required payment fields",
        code: "INVALID_PAYMENT_DATA",
        required_fields: ["reservation_id", "amount", "currency", "payment_method_id"]
      }), {
        status: 400,
    ...
    console.log(`🔍 [API] Payment request: Reservation ${paymentData.reservation_id}, Amount: ${paymentData.amount} ${paymentData.currency}`);
    ...
    const paymentResult = await (engine as any).processPayment(paymentData);
