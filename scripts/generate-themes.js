...
const DIRECTUS_TOKEN = process.env.DIRECTUS_ADMIN_TOKEN || 'rYncRSsu41KQQLvZYczPJyC8-8yzyED3';
...
${Object.entries(theme.layout || {}).map(([key, value]) => `  ${key}: ${value};`).join('\n')}

  /* Sizes from Directus */
${Object.entries(theme.sizes || {}).map(([key, value]) => `  ${key}: ${value};`).join('\n')}

  /* Effects from Directus */
${Object.entries(theme.effects || {}).map(([key, value]) => `  ${key}: ${value};`).join('\n')}
}`;
...
function generateFontsConfig(hotels) {
  ...
  for (const hotel of hotels) {
    ...
    if (fonts.detail) {
      ...
      if (fontName !== primaryName && fontName !== secondaryName && fontName !== monoName) {
        fontQueries.push(generateGoogleFontsURL(fontName, weight, style));
      }
    }
    ...
    fontsConfig.hotels[hotel.name] = {
      ...
      fonts: {
        ...
        secondary: fonts.secondary.name || fonts.secondary || 'Playfair Display',
...
function writeGeneratedFiles(themesCSS, fontsConfig) {
  ...
  writeFileSync(FONTS_CONFIG_PATH, JSON.stringify(fontsConfig, null, 2), 'utf8');
