...
// Load environment variables from .env file (only if not already set in system env)
...
async function _fetchHotelsFromDirectus() {
  console.warn(
    '⚠️ fetchHotelsFromDirectus() is deprecated in single-tenant mode. Use fetchCurrentHotelFromDirectus() instead.'
  );
...
async function generateThemes() {
  ...
  try {
    ...
    console.log(
      `✅ Single-tenant theme generation completed for: ${hotel.name}!`
    );
    console.log(
      `📊 Generated theme for hotel: ${hotel.name} (ID: ${hotel.id})`
    );
  } catch (error) {
